        ...
        <AnimatedSection>
          ...
          <h2 className="font-heading text-[clamp(2.25rem,4.5vw,3.5rem)] font-light text-midnight leading-[1.05] tracking-[-0.02em]">
            {content.title.includes("—") ? (
              <>{content.title.split("—")[0]}<br /><em className="italic text-gold font-normal">{content.title.split("—")[1]}</em></>
            ) : (
              content.title
            )}
          </h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-14">
          {content.items.map((t, i) => (
            <AnimatedSection key={i} delay={i * 0.08}>
              <div className="h-full bg-white rounded-[28px] p-8 border border-[#EDE8DC] flex flex-col gap-6 transition-all duration-500 hover:shadow-[0_24px_60px_rgba(0,0,0,0.08)] hover:-translate-y-1">
